...
const Card = (prop) => {
  ...
  const onDelete = async () => {
    const data = await axios.post(
      "https://ninja-fmcp.onrender.com/api/deleteAPI",
  ...
  return (
    <div className="card" key={prop.id}>
      ...
      <img
        ...
        name={prop.id}
        src={`data:image/png;base64,${prop.file}`}
        alt="API"
        onClick={onClickHandler}
      />
      <p className="card-title" name={prop.id} onClick={onClickHandler}>
        {prop.apiName}
      </p>
      <p className="card-description" name={prop.id} onClick={onClickHandler}>
        {prop.description}
      </p>
    </div>
  );
